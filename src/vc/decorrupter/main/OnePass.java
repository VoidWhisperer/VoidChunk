					...
					for(int z = 0; z < 32; z++)
					{
						...
						try
						{
							...
							if(!rootTag.hasKey("Level"))
							{
								...
								{
									NBTTagList tagList = levelTag.getList("Entities");
									int i = tagList.remove("all");
									levelTag.setList("Entities",tagList);
									System.out.println("Entities in chunk file at " + chunkX + "," + chunkZ + " removed: " + i);
									Main.out.append("Entities in chunk file at " + chunkX + "," + chunkZ + " removed: " + i);
									Main.out.newLine();
									//Main.out.append("Debug3");
									//Main.out.newLine();
									if(levelTag.getByteArray("Biomes") != null)
									{
										byte[] biomes = levelTag.getByteArray("Biomes");
										if(biomes.length < 256)
										{
											System.out.println("Corrupted chunk detected @ " + xPos + "," + zPos +". Fixing.");
											Main.out.append("Corrupted chunk detected @ " + xPos + "," + zPos +". Fixing.");
											Main.out.newLine();
											byte[] newBiomes = new byte[256];
											fill(newBiomes,(byte) -1);
											levelTag.setByteArray("Biomes", newBiomes);
											changed = true;
										}
									}
									NBTTagList tagList2 = levelTag.getList("Sections");
									for(int i2 = 0; i2 < tagList2.size(); i2++)
									{
										...
										Main.out.newLine();
										NBTTagCompound section = (NBTTagCompound) tagList2.get(i2);
